...
public:
    ...
    id_type id_of(const value_type& value) const {
       return { *components.key_of(value) };
    }

    id_type id_of(typename slot_map<value_type>::const_iterator iter) const {
        return { *components.key_of(iter) };
    }

    id_type peek_next_id() const { return { components.peek_next_key() }; }
};
...
public:
	...
	template<std::derived_from<T> Type>
	const Type& at(id_type id) const {
