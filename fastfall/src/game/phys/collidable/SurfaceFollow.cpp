...
{
    Vec2f n     = math::lefthand_unit_normal(line);
    ...
    , body_size     { collidable_size }
{
    ...
    if (debug::enabled(debug::Collision_Follow)) {
        ...
        lines[2].pos = curr_path.surface_line.p1 - math::lefthand_unit_normal(curr_path.surface_line) * depth;
        lines[3].pos = curr_path.surface_line.p1 - math::lefthand_unit_normal(curr_path.surface_line) * depth;
        lines[4].pos = curr_path.surface_line.p2;
        lines[5].pos = curr_path.surface_line.p2 - math::lefthand_unit_normal(curr_path.surface_line) * depth;
...
std::optional<size_t> SurfaceFollow::pick_surface_to_follow() const {
    ...
    Vec2f curr_dir = travel_dir * math::unit(curr_path.surface_line);

    for (auto& candidate : path_candidates)
    {
        auto dir_cand = travel_dir > 0.f ? candidate.surface_line : math::reverse(candidate.surface_line);
    ...
    if (curr_pick) {
        if (debug::enabled(debug::Collision_Follow)) {
            ...
            lines[2].pos = curr_pick->surface_line.p1 - math::lefthand_unit_normal(curr_pick->surface_line) * depth;
            lines[3].pos = curr_pick->surface_line.p1 - math::lefthand_unit_normal(curr_pick->surface_line) * depth;
            lines[4].pos = curr_pick->surface_line.p2;
            lines[5].pos = curr_pick->surface_line.p2 - math::lefthand_unit_normal(curr_pick->surface_line) * depth;
        }
    }
...
SurfaceFollow::Result SurfaceFollow::finish() {
    Vec2f unit = math::unit(curr_path.surface_line) * travel_dir;
...
bool SurfaceFollow::add_surface_if_valid(Linef path)
{
    ...
    Vec2f nan_inter{std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN()};
    Vec2f inter = dir_curr.p2 == dir_path.p1
                  ? dir_curr.p2
                  : math::intersection(dir_path, dir_curr).value_or(nan_inter);
    ...
    dir_curr = travel_dir > 0.f ? dir_curr : math::reverse(dir_curr);
    dir_path = travel_dir > 0.f ? dir_path : math::reverse(dir_path);
        ...
        if (collinear) {
            ...
            */
            ...
            if (path_type == surface_type::Ceil) {
                ...
                collinear_intersect += math::lefthand_unit_normal(path) * body_size.x * 0.5f;
            }
    ...
    Angle next_ang  = math::angle(dir_path);
    ...
    if (debug::enabled(debug::Collision_Follow)) {
        ...
        lines[0].pos = path.p1;
        lines[1].pos = path.p2;
        lines[2].pos = path.p1 - math::lefthand_unit_normal(path) * depth;
        lines[3].pos = path.p1 - math::lefthand_unit_normal(path) * depth;
        lines[4].pos = path.p2;
        lines[5].pos = path.p2 - math::lefthand_unit_normal(path) * depth;
