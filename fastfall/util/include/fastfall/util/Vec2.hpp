...
public:
	...
	/*
	constexpr Vec2<Type>& operator=(const sf::Vector2<Type>& other)
	{
		x = other.x;
		y = other.y;
		return *this;
	...
	*/
	...
	template <typename T>
	constexpr inline explicit Vec2(const Vec2<T>& vector) :
	...
	/*
	constexpr inline explicit Vec2(const sf::Vector2<Type>& vector) :
		x(vector.x),
		y(vector.y)
	{
...
inline Vec2<T>& operator +=(Vec2<T>& left, const Vec2<U>& right) {
...
template <typename T, typename U>
inline Vec2<T> operator +(const Vec2<T>& left, const Vec2<U>& right) {
...
template <typename T, typename U>
inline Vec2<U> operator *(T left, const Vec2<U>& right) {
	return Vec2<U>(right.x * left, right.y * left);
}

template <typename T, typename U>
inline Vec2<T>& operator *=(Vec2<T>& left, U right) {
	left.x *= right;
	left.y *= right;
	return left;
}
