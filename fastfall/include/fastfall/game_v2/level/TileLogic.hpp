...
class TileLogicType {
public:
	using FactoryFunction = std::function<std::unique_ptr<TileLogic>(World*)>;
...
public:
	...
	static void addType(const std::string& typeName) {
		TileLogicType type{
			typeName,
			[](World* w) -> std::unique_ptr<TileLogic> {
				return std::make_unique<T>(w);
			}
		};
		getMap().insert(std::make_pair(type.typeName, type));
	}
	static std::unique_ptr<TileLogic> create(World* world, std::string_view typeName);
