...
TEST(particle, emitter)
{
    ...
    emit.strategy.velocity_range = {75.f, 150.f};
    emit.strategy.emit_rate = {one_tick / 10, one_tick / 10};
    emit.strategy.max_particles = 200;
    emit.strategy.max_lifetime = one_tick * 40;

    ParticleRenderer render{emit, {-100, -100, 200, 100}};
    ...
    EXPECT_EQ(emit.particles.size(), 100);
